...
## sex vs. earn
ggplot(heights_df, aes(x=sex, y=earn)) + geom_point(color = 'red',alpha=0.3) + geom_boxplot(color = 'blue', fill = 'green', alpha = 0.3)
## race vs. earn
ggplot(heights_df, aes(x=race, y=earn)) + geom_point(color = 'red', alpha = 0.3)+ geom_boxplot(color = 'blue', fill = 'green',alpha = 0.3)

# https://ggplot2.tidyverse.org/reference/geom_bar.html
## Using `geom_bar()` plot a bar chart of the number of records for each `sex`
ggplot(heights_df, aes(sex)) + geom_bar(color = 'blue', fill='green',alpha=0.4)

## Using `geom_bar()` plot a bar chart of the number of records for each race
ggplot(heights_df, aes(race)) + geom_bar(color = 'blue', fill='green',alpha = 0.4)

## Create a horizontal bar chart by adding `coord_flip()` to the previous plot
ggplot(heights_df, aes(race)) + geom_bar(color = 'blue', fill='green',alpha = 0.4) + coord_flip()
...
ggplot(data=florida_df, aes(x=date, y=cases, group=1)) + geom_line(color = 'red')
